...
Lets tidy up the dataset into a data frame of only the variables which are required for the analysis.  
...
Follow up fungicide applications (cluster_2 and cluster_3) were also categorised as either, early, recommened or late.  
...
slim_PM_dat[slim_PM_dat$spray_management == "Late_plus",]
```
...
slimmer_PM_dat[slimmer_PM_dat$vi > 0.5,]

boxplot(PM_MB_means$prop_YG, na.rm = TRUE)

boxplot(PM_MB_means$yield_gain/PM_MB_means$grain_yield.t.ha., main = "Yield loss proportion")
summary(PM_MB_means$yield_gain/PM_MB_means$grain_yield.t.ha., na.rm = TRUE)
unique(PM_MB_means$trial_ref)

boxplot(yield_gain/grain_yield.t.ha. ~ year,data = PM_MB_means, main = "Yield loss proportion")

unique(PM_MB_means$fungicide_ai)
