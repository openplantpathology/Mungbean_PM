...
dat1[dat1$fungicide_ai == "control", c("yield_gain", "prop_YG")] <- c(NA,NA)
...
#   filter(fungicide_ai == "control")%>%
   ggplot(aes(x = grain_yield.t.ha, y = AUDPC_m))+
   geom_point()

summary(lm(grain_yield.t.ha ~ AUDPC_m, data = subset(slimmer_PM_dat, AUDPC_m <= 280)))

plot(grain_yield.t.ha. ~ AUDPC_m,data = subset(PM_MB_means, AUDPC_m <= 280) )
abline(lm(grain_yield.t.ha. ~ AUDPC_m, data = subset(PM_MB_means, AUDPC_m <= 280)))
...
The higher the yield the greater greater the influence on the disease on the yield especially if the crop is high yielding and a large AUDPC.  
...
# correct dates (this has been done in wrangle file but not updated yet in the data-set)
PM_MB_means[PM_MB_means$trial_ref == "mung1516/01", "harvest_date"] <- as.Date("2016-05-06", format = "%Y-%m-%d")
PM_MB_means[PM_MB_means$trial_ref == "mung1516/01", "final_assessment"] <- final_assessment = as.Date("2016-04-15", format = "%Y-%m-%d")
