...
for(i in levels(as.factor(PM_MB_means$Trial_ref))){
   ...
   if(any(is.na(unique(dat1$row_spacing)))){
      warning(paste(unique(dat1$Trial_ref), "at", unique(dat1$Location), "in year", unique(dat1$Year), "has unknown row.spacing (NA)\n 'yield_gain' and 'proportional yield gain' not calculated\n") )
      next()}
   
   for(j in unique(dat1$row_spacing)){
   
      dat2 <- dat1[dat1$row_spacing == j,]
      controlY <- mean(dat2[dat2$Fungicide == "control","grain_yield.t.ha."])
   
      dat2$yield_gain <- dat2$grain_yield.t.ha. - controlY
      dat2$prop_YG <- dat2$yield_gain/controlY
   
      dat2[dat2$Fungicide == "control",c("yield_gain", "prop_YG")] <- NA
...
controlY <- mean(dat1[dat1$Fungicide == "control","grain_yield.t.ha."])
...
write.csv(PM_MB_means, "data/1902 powdery mildew-Mungbean - Collated means.csv")
