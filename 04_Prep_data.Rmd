...
for (j in seq_along(PM_MB_dat[PM_MB_dat$trial_ref %in% Trials$trial_ref,]$yield_error)) {
   if (j == 1) {
      Sv <-
         vector(length = length(PM_MB_dat[PM_MB_dat$trial_ref %in% Trials$trial_ref,]$yield_error))
   }# create empty vector at first iteration with length of j
   
   Sv[j] = (PM_MB_dat[PM_MB_dat$trial_ref %in% Trials$trial_ref,]$yield_error[j]) ^ 2
}


# and replacing the values in the dataset
PM_MB_dat[PM_MB_dat$trial_ref %in% Trials$trial_ref,]$yield_error <-
   Sv
PM_MB_dat[PM_MB_dat$trial_ref %in% Trials$trial_ref,]$Y_error_type <-
   "SamplingVariance"
```
...
```{r}
data.frame(SprayTimingName = c("Early", "Recommended", "Late"),
           DaysRelativeToFirstSign = c("Prior to first sign of PM",
                                       "1 - 3 days after first sign of PM",
                                       "7 - 8 days after first sign of PM"))%>%
   kable()
```
