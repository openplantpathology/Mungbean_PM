...
`r round(PM_mv$ci.lb[6],5)*1000` to `r round(PM_mv$ci.ub[6],5)*1000` kg ha^-1^ in the $LatePlus$ treatments (`r reportP(PM_mv$pval[6])`) (Figure \@ref(fig:Figure2)), which again follows the advice given by Thompson et al. and Sparks and Kelly [-@SueThompson2016; -@Sparks2017].
This meta-analysis supports the advice, which is currently given to growers, especially around any uncertainty that early sprays might be effective at saving yield. 
We show here early fungicide applications made prior to any sign of the disease were not effective at mitigating yield loss.
...
Growers typically do not have the time to inspect their crop regularly for the first sign of powdery mildew and therefore the disease may not be spotted within the first couple of days of it becoming established.
...
These issues, and perhaps other on-farm duties, might delay a timely application permitting the disease more time to become established.
Knowing a delay of up to two weeks may not lead to yield losses, can allow growers to incorporate other integrated pest management options, such as an insecticide application with the fungicide saving on application costs.
Second applications, if they are needed, should be applied at least 10 -- 14 days after the first application and not within 21 days of harvest under the currently labelled permit [@APVMAcustodia].
If time permits a second spray after the first "late" application, on average the meta-analysis shows this would be optimal, as the $LatePlus$ treatment significantly limited yield loss by `r round(T3$estimates[10], 4)*1000` kg ha^-1^, when compared with the $Late$, single spray treatment `r reportP(T3$pval[10])`.
In all cases, label requirements should be consulted and strictly adhered to when planning a spray schedule.

It is important to note that the studies included in this meta-analysis might not reflect a "normal" mungbean season.
All the included experiments were intentionally sown late in the mungbean growing season from January through early March to ensure powdery mildew infection (Figure \@ref(fig:Figure2)).
...
An example of this can be shown by inspecting the six trials in which powdery mildew manifested 60 days or greater after planting (Trial references: AM1304, mung1819/01, mung1011/01, AM1303, AM1305 and BB1305). 
...
This indicated that there is likely no benefit to applying two fungicide applications when the disease manifests later than 60 days into the growing season.
Furthermore the trials undertaken in 2013, Premer (AM1303), Goolhi (AM1305) Marys Mount (AM1304) and Millmerran (BB1305), reported a significant difference in powdery mildew severity between the fungicide treatments.
...
Therefore, as discretion is still required to determine if fungicide applications are necessary given the crop growth stage and forecast weather conditions, the PowderyMildewMBM considers crop growth stage and weather to assist decision making whether to spray or not to spray [@Diggle].
...
The meta-analysis estimates average yield savings of between `r coef(PM_mv)[3]*1000` kg ha^-1^ ($Recommended$) and `r coef(PM_mv)[4]*1000` kg ha^-1^ ($RecommendedPlus$). 
