...
PM_MB_means <- 
   ...
   mutate(pod_final_date = as.Date(pod_final_date)) %>% 
   mutate(mid_late_pod_final = as.Date(mid_late_pod_final)) %>% 
   mutate(first_sign_disease = as.Date(first_sign_disease)) %>% 
   mutate(fungicide_application_1 = as.Date(fungicide_application_1)) %>% 
   mutate(fungicide_application_2 = as.Date(fungicide_application_2)) %>% 
   mutate(fungicide_application_3 = as.Date(fungicide_application_3)) %>%
   mutate(fungicide_application_4 = as.Date(fungicide_application_4)) %>% 
   mutate(fungicide_application_5 = as.Date(fungicide_application_5)) %>%
   mutate(fungicide_application_6 = as.Date(fungicide_application_6)) %>%
   mutate(fungicide_application_7 = as.Date(fungicide_application_7)) %>% 
   mutate(harvest_date = as.Date(harvest_date)) %>% 
   mutate(final_assessment = as.Date(final_assessment))
...
Perhaps Amistar Xtra and Custodia might be able to be pooled given they both contain strobilurin and triazole, however they contain differing dose ratio (inverted).  
...
Dose of the active ingredient ranged from 62.35 grams per hectare to 60 grams per hectare. 
...
PM_MB_trianLMER.4 <-
   lmer(Yield_error ~ `grain_yield.t.ha.` + as.factor(Fungicide) +
           (1 | trial_ref),
        data = PM_MB_train)
