...
Now for the tricy part creating 'Sampling Variance' for treatments where only mean yield is recorded.  
...
Two trials `mung1516/03` and `mung1112/01` within the scope of the meta-analysis fail to report variance statistics. According to [Machado F. et. al (2017)](http://apsjournals.apsnet.org/doi/10.1094/PDIS-03-17-0340-RE) [Paul et al. (2007)](https://apsjournals.apsnet.org/doi/pdfplus/10.1094/PHYTO-97-2-0211) we can use a linear regression model to estimate sampling variance.  
...
We need to impute the variances which are missing for a few of the trials. These trials `r unique(slimmer_PM_dat[is.na(slimmer_PM_dat$yield_error),c("location","year", "trial_ref")])` were analysed and reported that there was no significant difference between the treatments in each trial. So therefore we need the imputed variances not to show a significant difference.  
...
Finally we will assign factor classes to the main variables in the meta-analysis.  
```{r meta_organisation}
library(metafor)
