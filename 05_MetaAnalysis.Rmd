...
abline(v = lm_data[lm_data$P_value == min(lm_data$P_value),"days_from_plant"])
plot(r_squared ~ days_from_plant, data = lm_data)
abline(v = lm_data[lm_data$r_squared == max(lm_data$r_squared),"days_from_plant"])
...
for(j in -20:20){
   ...
   for(i in seq_along(rain_dat_sum$trial_ref)){
         ...
         rain_dat_sum$rainfall_sum[i] <- 
            AUS_rainfall(StartDate = (as.Date(rain_dat_sum$planting_date)[i] + 11),
                         EndDate = (as.Date(rain_dat_sum$planting_date)[i] + 90 + j),
                         lat = rain_dat_sum$lat[i],
                         long = rain_dat_sum$lon[i])
...
abline(v = -3)
abline(h = 0.00315, col = "grey")
plot(r_squared ~ days_from_plant, data = lm_data)
abline(v = -3)
abline(h = 0.562, col = "grey")
plot(estimate ~ days_from_plant, data = lm_data)
abline(v = -3)
...
source("R/year2day.r")
year.day(season_dates$Planting_date[5])
