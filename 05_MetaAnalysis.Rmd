...
The trial, `mung1112/02` uses Least squre differences to describe the variation within the experiment. The following code attempts to convert this to sampling variance as per the method in 
...
n.B <- PM_MB_means[PM_MB_means$Trial_ref == "mung1112/02",]$Replicates[2]
...
MSE <- (LSD/Tcrit)^2/(1/n.A + 1/n.B)
MSE <- PM_MB_means[PM_MB_means$Trial_ref == "mung1112/02",]$Replicates[1]*(LSD/Tcrit)^2/2 # method for finding variance in Nugugi et.al (2011) DOI: 10.1094/phyto-08-10-0221

```
![calculating sampling variance from mean squared error Paul et. al. (2008)](Paul_etal_2008.PNG)

```{r sampling_variance}
for(j in seq_along(PM_MB_means[PM_MB_means$Trial_ref == "mung1112/02",]$Replicates)){
   if(j == 1){Sv <- vector(length = length(PM_MB_means[PM_MB_means$Trial_ref == "mung1112/02",]$Replicates))}
#  Sv[j]^2 = MSE/(PM_MB_means[PM_MB_means$Trial_ref == "mung1112/02",]$Replicates[j] *
#                   PM_MB_means[PM_MB_means$Trial_ref == "mung1112/02",]$grain_yield.t.ha.[j])
# Balance equation 
   Sv[j] = sqrt(MSE/(PM_MB_means[PM_MB_means$Trial_ref == "mung1112/02",]$Replicates[j] *
                  as.numeric(PM_MB_means[PM_MB_means$Trial_ref == "mung1112/02",]$grain_yield.t.ha.[j])))

   }

# and replacing the values in the dataset

PM_MB_means[PM_MB_means$Trial_ref == "mung1112/02",]$Yield_error <- Sv
PM_MB_means[PM_MB_means$Trial_ref == "mung1112/02",]$Y_error.type <- "SamplingVariance"
...
# Equation to convert standard deviation to Sampling Variance 
   Sv[j] = (PM_MB_means[PM_MB_means$Trial_ref %in% Trials$Trial_ref,]$Yield_error/
               sqrt(PM_MB_means[PM_MB_means$Trial_ref %in% Trials$Trial_ref,]$Replicates))^2
   }
