...
In crop rain could be a significant factor which contributes to the end of season grain yield. To investigate any relationship I will loop a linear model over time periods from 10 days prior to planting, to 100 days after planting.

```{r yield_from_rain_DataRangle}
...
Loop over the dates at the start of the season (to 90 days after planting) to find which date fits the best linear model for grain weight and in-crop rainfall.
...
for(j in -9:30){
   ...
   lm_rain <- data.frame(start_day = rep(-20:40, times = length(-30:30)),
              end_day = rep(-30:30, each = length(-20:40)),
              lm_pval = NA,
              lm_rsquared = NA,
              lm_adj_rsquared = NA
   )
...
# clusterExport(cl8, list("rain_dat_sum", "AUS_rainfall", "lm_rain", "get_historical"))
   ...
   testlist1 <- apply(lm_rain,1,function(x1){
         ...
         rain_dat_sum$rainfall_sum <- AUS_rainfall(StartDate = as.Date(rain_dat_sum$planting_date)[i] + x1[1],
...
head(testlist1)   
lm_rain$lm_pval <- head(bind_rows(testlist1))
   ?bind_rows
...
summary(lm_rain$lm_adj_rsquared)
...
ggplot(lm_rain, aes(x = start_day, y = end_day, z = lm_pval))+
   ...
   if(k == -19 & j == -19){
      lm_pval <- matrix(nrow = 40,
                        ncol = 40)
      colnames(lm_pval) <- as.character(-19:20) # End date
      row.names(lm_pval) <- as.character(-9:30) # start date
      
      
      lm_square <- matrix(nrow = 40,
                          ncol = 40)
      colnames(lm_square) <- as.character(-19:20) # End date
      row.names(lm_square) <- as.character(-9:30) # start date
      }
   
   for(i in seq_along(rain_dat_sum$trial_ref)){
      
         rain_dat_sum$rainfall_sum[i] <- 
            AUS_rainfall(StartDate = (as.Date(rain_dat_sum$planting_date)[i] + j),
                         EndDate = (as.Date(rain_dat_sum$planting_date)[i] + 80 + k ),
                         lat = rain_dat_sum$lat[i],
                         long = rain_dat_sum$lon[i])
    ...
    lmod1 <- summary(lm(rainfall_sum ~ grain_yield, 
      ...
      lm_pval[j,k] <- coef(lmod1)[2,4]
      lm_square[j,k] <- lmod1$r.squared
      
   
   }
   }# If it aborts with an error Error in row_start:(row_start + season) : argument of length 0, just re-run the code
   
image(1:ncol(lm_pval), 1:nrow(lm_pval), t(lm_pval), col = terrain.colors(60), axes = TRUE)

image(1:ncol(lm_square), 1:nrow(lm_square), t(lm_square), col = terrain.colors(60), axes = TRUE)

ggplot(lm_pval, aes(x = colnames(lm_pval), y = row.names(lm_pval), z = ))
...
#### Best end day for in-crop rainfall

```{r rainfall_sum_SEnd}

# Loop to find the last day in the season for which in crop rainfall is important
for(j in -20:20){
   if(j == -20){lm_data <- data.frame()}
   
   for(i in seq_along(rain_dat_sum$trial_ref)){
      
         rain_dat_sum$rainfall_sum[i] <- 
            AUS_rainfall(StartDate = (as.Date(rain_dat_sum$planting_date)[i] + 14),
                         EndDate = (as.Date(rain_dat_sum$planting_date)[i] + 80 + j),
                         lat = rain_dat_sum$lat[i],
                         long = rain_dat_sum$lon[i])
    ...
    lmod1 <- summary(lm(rainfall_sum ~ grain_yield, 
      ...
      lm_data <- rbind(lm_data,
                    data.frame(days_from_plant = j + 80,
...
plot(log(P_value) ~ days_from_plant, data = lm_data, xlab = "End of in-crop rain referenced to 90 days after planting")
abline(v = lm_data[lm_data$P_value == min(lm_data$P_value),"days_from_plant"], col = "grey")
text(x = lm_data[lm_data$P_value == min(lm_data$P_value),"days_from_plant"], 
     y = max(log(lm_data$P_value * 0.75)), 
     labels = paste("pvalue =",round(min(lm_data$P_value),4)),
     pos = 4)

plot(r_squared ~ days_from_plant, data = lm_data, xlab = "End of in-crop rain referenced to 90 days after planting")
abline(v = lm_data[lm_data$r_squared == max(lm_data$r_squared),"days_from_plant"], col = "grey")
text(x = lm_data[lm_data$r_squared == max(lm_data$r_squared),"days_from_plant"], 
     y = max(lm_data$r_squared * 0.75), 
     labels = paste("Rsquared =",round(max(lm_data$r_squared),4)),
...
plot(estimate ~ days_from_plant, data = lm_data)
abline(v = -3 ,h = lm_data[lm_data$r_squared == max(lm_data$r_squared),"estimate"], col = "grey")


#write.csv(lm_data, "data/rainfall_x_Gyield_lmod.csv")

lm_data <- read.csv("data/rainfall_x_Gyield_lmod.csv", stringsAsFactors = FALSE)
