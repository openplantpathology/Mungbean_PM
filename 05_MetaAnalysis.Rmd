...
## AUPDC and PM severity meta-analysis

```{r LM_AUDPC}
library(MuMIn)
summary(lm(grain_yield.t.ha ~ AUDPC_m, data = slimmer_PM_dat))
summary(lm(grain_yield.t.ha ~ AUDPS_m, data = slimmer_PM_dat))
```

Area under the disease progress stairs (AUDPS) shows almost the same fit as the AUDPC linear model.
...
AUDPC becomes a significant predictor of grain yield when the random effect `trial` is accounted for. However we noticed in the plots earlier that not only the intercept but the slope varies depending on the trial. Lets see if the model improves when AUDPC is used as a random slope for trial.  
...
```{r AUDPCvsAUPDS}
r.squaredGLMM(Me_mod2.s)
r.squaredGLMM(Me_mod2)
```

The R-squared of the AUDPC model is a better fit compared to the AUDPS model. From here on we will use AUDPC
...
Powdery mildew incidence at the end of the season was a significant covariate for grain yield, however was a weeker predictor compared to AUDPC. In fact a model comparison shows AUDPC is significantly better at predicting grain yield.   
...
Also there is no reason to include these terms individually in the model to assess their independant effect on grain yield as we know the application of fungicide to mitigate yield loss is aimed at reducing the disease in the crop.  
...
```
As we would hope this improves out model again. Not only do we want to know how much spray_management moderates the effect disease (AUDPC) on yield. We might also want to know on average what effect did each of the spray management have on yield.

```{r AUDPCplusspraymangement}
Me_mod7 <- lmer(grain_yield.t.ha ~ AUDPC_m : spray_management+ spray_management + (AUDPC_m|trial),
              data = slimmer_PM_dat)

anova(Me_mod6, Me_mod7)
```
The model with an additive spray_management term did not significantly improve the model, however we don't need the term in the model to answer our hypothesis. We can still determine the effect of spray_management on yields through the interactive AUDPC:spray_management term.  
...
```
As expected the model produced an error warning theat the maximum number of evaluations were exceeded. Also the model comparisons show that the newer Me_mod7 was not a better model. We we proceed with Me_mod6 as the model with the best fit.
...
Area under the disease progress curve (AUDPC) shows no linear relationship with the grain yield data when analysed without respect to any other factor. Lets use a mixed effect model which incorporates `trial` as a random effect. This makes sense given when we plotted the data subset by trial we could see a better relationship between grain yield and AUDPC.
...
```
The model is significantly improved, lower AIC and a higher log likelihood. 

```{r LMER_AUDPC2_results}
summary(Me_mod2)
coef(Me_mod2)[["trial"]][
   order(coef(Me_mod2)[["trial"]]$AUDPC_m),]
```
Here the intercept represents an estimate of the trial yield without the limitation of the disease. AUDPC_m shows the slope of the regression for grain yield as AUDPC increases. The coefficients of the model (ordered by AUDPC) show that higher yielding trials had a steeper negative slope. In other words higher yielding mungbean crops were likely to see greater losses from disease.
...
Powdery mildew incidence at the end of the season was a significant covariate for grain yield, however was a weeker predictor compared to AUDPC.  
...
Me_mod7 <- lmer(grain_yield.t.ha ~  AUDPC_m : spray_management+ (AUDPC_m|trial),
              data = slimmer_PM_dat)
