...
There seems like no difference between the treatments, with exception to Late_plus. Lets do this plo again, but use the proportion of yield increase compared to the no spray control as the response variable.  
...
When we compare these results to those of the linear mixed effect model we notice the metafor model is less certain about the `recommended` and `recommended_plus` treatments. Subsequently there is no signiciant difference between the `recommended` and `recommended_plus` treatments and the no spray control.
...
To make it easier to compare each of the treatments we can compute the contrasts.
```{r meta_contrasts}
...
What is interesting here is that the variance in the control has increased while the treatments have decreased. This is despite the control being the best represented accross all experiments. However it could also be because it also has a low number of pooled reps per trial.  
