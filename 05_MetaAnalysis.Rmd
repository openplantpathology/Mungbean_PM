...
We need to impute the variances
...
again <- 0
for(i in seq(from = again, to = 500, by = 0.1)){
if(i == 0){log_Yerror <- vector()}
   
      
   x1 <- rnorm(1, mean = mean(log(slimmer_PM_dat$yield_error), na.rm = TRUE), #+ i,
                  sd = sd(log(slimmer_PM_dat$yield_error), na.rm = TRUE)) 
   if(x1 <= -7){again <- i
      next()}
   log_Yerror <- c(log_Yerror,x1)
   
   if(length(log_Yerror) < 14){again <- i
      next()}
   
   if(length(log_Yerror) > 14){ stop(message("length of log_Yerror exceeded 14"))}
...
if(max(outer(X = slimmer_PM_dat[is.na(slimmer_PM_dat$yield_error),"grain_yield.t.ha"] - (2*slimmer_PM_dat[is.na(slimmer_PM_dat$yield_error),"yield_SE"]),
   ...
   next()
                ...
                message(paste(#"\nA value of",i,"was added to the mean distribution from which imputed erorrs were sampled. This led to 
...
# slimmer_PM_dat[is.na(slimmer_PM_dat$),] %>%
...
slimmer_PM_dat$yi <- slimmer_PM_dat$grain_yield.t.ha
...
```{r meta_2}
# Using trial instead of trial_ref

head(slimmer_PM_dat)

slimmer_PM_dat$yi <- log(slimmer_PM_dat$grain_yield.t.ha)
slimmer_PM_dat$vi <- log(slimmer_PM_dat$pop_var)
slimmer_PM_dat$spray_management <- factor(slimmer_PM_dat$spray_management)
...
```{r inspect_for_outliers}
head(slimmer_PM_dat)

slimmer_PM_dat %>%
#   filter(spray_management == "Late_plus") %>%
   ggplot(aes(prop_yield_gain, fill = spray_management))+
   geom_histogram(alpha = 0.5)
   
slimmer_PM_dat %>%
   filter(spray_management == "Late_plus") %>%
   ggplot(aes(prop_yield_gain, fill = trial_ref))+
   geom_histogram(alpha = 0.5)



slimmer_PM_dat %>%
   filter(spray_management == "Recommended") %>%
   ggplot(aes(prop_yield_gain, fill = trial_ref))+
   geom_histogram(alpha = 0.5)
