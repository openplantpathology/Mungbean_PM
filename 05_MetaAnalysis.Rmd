...
plot(PM_MB_train$Yield_error, log(as.numeric(PM_MB_train$`grain_yield.t.ha.`)))


library(lme4)

PM_MB_trianLMER <- lmer(log(as.numeric(`grain_yield.t.ha.`)) ~ Yield_error + 
         (1|Trial_ref), data = PM_MB_train)

paramA <- (summary(PM_MB_trianLMER)$coef)[2,1]
InterC <- (summary(PM_MB_trianLMER)$coef)[1,1]

ggplot(PM_MB_train, aes(y = log(as.numeric(`grain_yield.t.ha.`)), x = Yield_error, colour = Trial_ref))+
   geom_point()+
   geom_abline(intercept = InterC, slope = paramA)


exp((log(as.numeric(PM_MB_means[PM_MB_means$Trial_ref == "mung1516/03",]$`grain_yield.t.ha.`)) * paramA)+InterC)
# PM_MB_means[PM_MB_means$Trial_ref == "mung1516/03",]$Yield_error
hist(PM_MB_train$Yield_error)
hist(as.numeric(PM_MB_train$`grain_yield.t.ha.`))
