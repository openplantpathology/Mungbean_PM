...
PM_MB_means_T <- PM_MB_means %>%
   ...
   summarise(trial_mean = mean(grain_yield.t.ha.,na.rm = TRUE),
             Trial_variance = sd(grain_yield.t.ha.,na.rm = TRUE)^2)  # I am not sure I have calculated between trial variance or trial variance// Should I be using a denominator (N-1) or (N), sd by default uses (N-1)
...
for(i in SV_Trials){
   PM_MB_means[PM_MB_means$Trial_ref == i, "TrialVariance"] <- PM_MB_means_T[PM_MB_means_T$Trial_ref == i,"Trial_variance"]
}
