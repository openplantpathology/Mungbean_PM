AUS_rainfall <- function(StartDate = Sys.Date() - 30,
   ...
   if (is.null(latlong)) {
      ...
      if (sum(temp_files == paste0("IDCJAC0009_", 
                                   StationID, "_1800_Data.csv")) > 0) {
         rain <-
            read.csv(paste0(
               tempdir(),
               "\\IDCJAC0009_",
         ...
         rain <-
            as.data.frame(bomrang::get_historical(stationid = StationID,
                                                  type = c("rain")))
      }
      ...
      if (sum(list.files(tempdir()) == "cache_rain.csv") > 0) {
         ...
         if (length(cache_rain[cache_rain$latt == as.numeric(latlong[1]) &
            ...
            temp_files <- list.files(tempdir())
            if (sum(temp_files == paste0("IDCJAC0009_", StationID,
                                         "_1800_Data.csv")) > 0) {
               rain <-
                  read.csv(
                     paste0(
            ...
            if (sum(temp_files == paste0("IDCJAC0009_", StationID,
                                         "_1800_Data.csv")) == 0) {
               rain <-
                  as.data.frame(bomrang::get_historical(
                     latlon = as.numeric(latlong) ,
                     type = c("rain")
                  ))
            }
   ...
   if (nchar(rain$Bureau.of.Meteorology.station.number[1]) == 5) {
      rain$Bureau.of.Meteorology.station.number[1] <-
         paste0("0", rain$Bureau.of.Meteorology.station.number[1])
   }
   if (nchar(rain$Bureau.of.Meteorology.station.number[1]) == 4) {
      rain$Bureau.of.Meteorology.station.number[1] <-
         paste0("00", rain$Bureau.of.Meteorology.station.number[1])
   }
   ...
   cache_rain_tmp <-
      data.frame(
         Product.code = as.character(rain[1, 1]),
         BOM.station.number =
   ...
   write.csv(cache_rain,
             paste0(tempdir(), "\\cache_rain.csv"),
             row.names = FALSE)
   ...
   Date1 <-
      as.numeric(unlist(strsplit(as.character(StartDate), "-")))
   ...
   rainfall <- sum(rain[as.numeric(row_start):(as.numeric(row_start) +
                                                  as.numeric(season)),
                        6], na.rm = TRUE)
