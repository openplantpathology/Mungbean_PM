...
Herm_16_m[,"yield_error"] <- 
   Herm_16_dat %>%
   group_by(n_treatment) %>%
   summarise(PM_final_severity = sd(PM_final_severity, na.rm = TRUE),
             yield_error = sd(grain_yield.t.ha., na.rm = TRUE)) %>%
...
# could also use gsub to remove Ts
...
# COULD NOT GET TO MATCH
# insert new calculations into data
# #PM_MB_means <- 
   # replace_data(template_data = PM_MB_means,
   #                          new_data = Herm_17_m,
   #                          new_data_column = "AUDPC_m",
   #                          filter_cols = c("trial_ref","grain_yield.t.ha.","year","location",
   #                                          "host_genotype","n_treatment", "fungicide_ai","row_spacing"))

# PM_MB_means <- replace_data(template_data = PM_MB_means,
#                             new_data = Herm_17_m,
#                             new_data_column = "AUDPC_sd",
#                             ignore_columns = c(5:8,10:16,29:46))
...
#PM_MB_means <- 
   replace_data(template_data = PM_MB_means, 
                            new_data = Well_18_m, 
                            new_data_column = c("AUDPC_m", "AUDPC_sd"),#"AUDPS_m", "AUDPS_sd","Inc_Ms"),
                            filter_cols = c("trial_ref","year","location","n_treatment"))

dim(Well_18_m) ==
dim(PM_MB_means[PM_MB_means$trial_ref == "mung1718/01",])   

   # Not sure why replace data will not replcae the following columns
