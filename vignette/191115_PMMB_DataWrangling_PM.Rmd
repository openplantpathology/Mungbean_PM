...
fold_data <- function(new_DataFrame, template_DataFrame) {
   ...
   # If colums are missing from the new_DataFrame that are in the template_DataFrame, it creates them 
   # with NA values. 
   ...
   # Using this function assists in joining the two data.frames together so colums are in the same order
   ...
   new_DataFrame <-
      new_DataFrame[, c(cnames2, cnames1[is.na(x1)])]  # adding columns to new_DataFrame not in template_DataFrame  # aranges column names to new_DataFrame not in template_DataFrame
   
   return(new_DataFrame)
}
source(here("R/copy_class.R"))
source(here("R/data_mesh.R"))
```
...
# I have comment out this code because it does not replace the control plots properly
# insert new calculations into data
# PM_MB_means <-   replace_data(template_data = PM_MB_means, 
...
PM_MB_means <- 
   replace_data(template_data = PM_MB_means, 
                            new_data = Well_18_m, 
                            new_data_column = c("AUDPC_m", "AUDPC_sd"),
                            filter_cols = c("trial_ref","year","location","n_treatment"))

# check the same dimensions
dim(Well_18_m) ==
dim(PM_MB_means[PM_MB_means$trial_ref == "mung1718/01",])   

   # Not sure why replace_data will not replcae the following columns
   PM_MB_means[c(324: 347),
               c("AUDPS_m", "AUDPS_sd","Inc_Ms")] #<-
      Well_18_m[,c("AUDPS_m", "AUDPS_sd","Inc_Ms")]
...
```{r Save_PM_MB_means, eval=FALSE, include=FALSE}
