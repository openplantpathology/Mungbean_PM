...
Goolhi_13_m <- Goolhi_13_dat %>%
   ...
   summarise(plot_length.m. = mean(plot_length, na.rm = TRUE),
             ...
             AUDPC_m = mean(AUDPC),
             AUDPC_sd = sd(AUDPC),
              Y_Msquare = anova(lm(grain_yield.t.ha. ~ factor(total_fungicide), 
...
Herm_15_m <- Herm_15_dat %>%
   ...
   summarise(trial_ref = unique(trial_ref),
             ...
             AUDPC_m = mean(AUDPC),
             AUDPC_sd = sd(AUDPC),
             Y_Msquare = anova(lm(grain_yield.t.ha. ~ factor(n_treatment), data = Herm_15_dat))[1,3]
   )
...
# #PM_MB_means <- 
   ...
   #                          new_data = Herm_17_m,
   #                          new_data_column = "AUDPC_m",
   #                          ignore_columns = c(4,5,7,8,10:16,29:46))
...
PM_MB_19[is.na(PM_MB_19$`1-5-19.Incedence`),"AUDPC"] <- 
   apply(data.frame(zero = 0,PM_MB_19[is.na(PM_MB_19$`1-5-19.Incedence`) ,c("15-4-19.Incedence", "23-4-19.Incedence")]-1),1,FUN = audpc, dates = c(60, 70,78))
```
