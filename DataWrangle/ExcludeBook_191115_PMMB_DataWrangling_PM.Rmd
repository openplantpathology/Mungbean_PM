...
Herm_10_m <- Herm_10_dat %>%
   ...
   summarise(
      ...
      AUDPS_sd = sd(AUDPS),
      Inc_Ms = anova(lm(
         AUDPC ~ host_genotype + factor(n_treatment), data = Herm_10_dat
      ))[2, 3],
      Y_Msquare = anova(
         lm(grain_yield.t.ha. ~ host_genotype + factor(n_treatment), data = Herm_10_dat)
      )[2, 3]
...
Herm_10_m[order(Herm_10_m$n_treatment), "disease_error"] <-
   Herm_tentmp[order(Herm_tentmp$n_treatment), "disease_error"]
Herm_10_m[order(Herm_10_m$n_treatment), "yield_error"] <-
   Herm_tentmp[order(Herm_tentmp$n_treatment), "yield_error"]
...
PM_MB_means <- replace_data(
   template_data = PM_MB_means,
   new_data = Herm_10_m,
   new_data_column = c(
      "AUDPC_m",
      "AUDPC_sd",
...
write.csv(
   ...
   "../data/Past trials/2010/2010 PMmung Herm/2010 PMmung Hermitage means.csv"
)
...
write.csv(
   King_11_m,
   "C:/Users/U8011054/USQ/SCP - Documents/DAW1810/mungbean_powdery-mildew/Past trials/2011/2011 PMmung King/2011 PMmung Kingaroy means.csv"
)
...
```{r Save_PM_MB_means, eval=FALSE}

write.csv(PM_MB_means, file = "../data/PM_MB_updated.csv")
