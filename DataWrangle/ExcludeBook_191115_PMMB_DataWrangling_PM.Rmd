...
source("C:/Users/U8011054/OneDrive - USQ/Cloudstor/R/Mungbean_projects/Mungbean_PM/R/import_data.R")
...
King_16_m <- King_16_dat %>%
   ...
   summarise(trial_ref = unique(trial_ref),
             ...
             #AUDPC.m = mean(AUDPC),
             #AUDPC.sd = sd(AUDPC),
             meanSq = anova(lm(grain_yield.t.ha. ~ factor(n_treatment), data = King_16_dat))[1,3]
   )
...
King_16_m$AUDPC.m <- apply(King_16_m[,c("dae20", "dae34", "dae46", "PM_final_severity")]-1,1, 
                            FUN = audpc, dates = c(1,14,26,40))
...
head(Fogerty_17)
unique(Fogerty_17$Chem.Trt)
unique(Fogerty_17$Chemical)
Fogerty_17[,7:11]
```
...
Fogerty_17_dat <- Fogerty_17 %>%
   mutate(trade_name = 
             case_when(Chem.Trt == 1 ~ "Folicur",
                       ...
                       TRUE ~ "other"))%>%
   mutate(fungicide_ai = 
             case_when(Chem.Trt == 1 ~ "tebuconazole",
...
Fogerty_17_dat <- 
   data.frame(trial_ref = "mung1617/02",
              ...
              fungicide_ai = Fogerty_17_dat$fungicide_ai,
...
Fogerty_17_m <- Fogerty_17_dat %>%
   ...
   summarise(trial_ref = unique(trial_ref),
             ...
             AUDPC.m = mean(AUDPC),
             AUDPC.sd = sd(AUDPC),
             meanSq = anova(lm(grain_yield.t.ha. ~ factor(n_treatment) + fungicide_ai + row_spacing, data = subset(Fogerty_17_dat, trade_name != "Custodia")))[1,3] # of only triazoles
   )
...
Herm_17_dat <- 
   data.frame(trial_ref = "mung1617/01",
              ...
              pod_fill_date = as.Date(NA),
              mid_late_pod_fill = as.Date(NA),
              first_sign_disease = as.Date("2017-03-13", format = "%Y-%m-%d"),
...
Herm_17_sum <- Herm_17 %>%
   ...
   summarise(
      ...
      meanSq = anova(lm(`Yield.T/Ha` ~ factor(Manage.Trt) + factor(Trt) + `Row.Spacing.(m)`, data = subset(Herm_17, Chemical != "Custodia® 320 SC")))[1,3] # of only triazoles
   )

Herm_17_sum <-
   with(Herm_17_sum, Herm_17_sum[order(Manage.Trt, Chemical, `Row.Spacing.(m)`), ])

# inserted meansquare in manually
...
# Columns to keep
Well_18 <- Well_18[, c('Plot','Rep','Plant.Pop','Row.Spacing',
            'Fungicide','Treatment','IR.23-3-18',
            'IR.5-4-18','IR.11-4-18','IR.18-4-18',
            'IR.26-4-18','IR.2-5-18','S.23-3-18',
            'S.5-4-18','S.11-4-18','S.18-4-18',
            'S.26-4-18','S.2-5-18','Row.Length','Yield./.ha')]


Well_18_means <- Well_18 %>%
...
Well_18$FungVCon <- Well_18$Fung.Trt
Well_18[Well_18$FungVCon == 1,"FungVCon"] <- 3
Well_18[Well_18$FungVCon == 4,"FungVCon"] <- 1
Well_18[Well_18$FungVCon == 3,"FungVCon"] <- 2
...
Well_18_means <- Well_18.1 %>%
   ...
   summarise(
      ...
      Incidence  = max(Well_18.1$Incidence),
      Severity  = max(Severity, na.rm = TRUE),
      meanSq = anova(lm(`Yield./.ha` ~ factor(Plant.Pop) + factor(FungVCon) + `Row.Spacing`, data = subset(Well_18, Fungicide != "Custodia")))[2,3] # of only triazoles
   )
...
PM_MB_19$AUDPC <- apply(PM_MB_19[,c("15-4-19.Incedence", "23-4-19.Incedence", "1-5-19.Incedence")]-1,1,FUN = audpc, dates = c(3,8,21))
...
PM19_means <- PM_MB_19 %>%
   ...
   summarise(
      ...
      Yield = mean(Yeild/1000, na.rm = TRUE),
      Yield_error = sd(Yeild/1000, na.rm = TRUE),
      AUDPC = mean(AUDPC, na.rm = TRUE),
      meanSq = anova(lm(Yeild/1000 ~ factor(TOS.NUM) + factor(`T/ment.NUM`), data = PM_MB_19))[2,3]
   )
