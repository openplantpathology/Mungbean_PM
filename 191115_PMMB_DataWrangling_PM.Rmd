...
Herm_15_dat <- 
   data.frame(trial_ref = "mung1415/01",
              ...
              harvest_date = as.Date("2013-04-20", format = "%Y-%m-%d"),
              final_assessment = as.Date("2013-04-06", format = "%Y-%m-%d"),
              PM_final_severity = Herm_15_dat$Inc_42D,
              rating_scale = "1-9",
              grain_yield.t.ha. = as.numeric(Herm_15_dat$`kg/Ha`)/1000,
              AUDPC = apply(Herm_15_dat[,c("Inc_23D","Inc_42D")]-1,1, FUN = audpc, dates = c(23,42)) # -1 to the data because zero disease = 1 on rating scale// Dates are based off days from first sign
              )
