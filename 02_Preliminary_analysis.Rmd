...
fungicide_spray_plot <- 
   PM_MB_dat %>%
   filter(trial_ref %in% DMITrials)%>%  
   mutate(spray_treat = ifelse(fungicide_ai == "control","control", "DMI spray"))%>%
   group_by(trial_ref, location, year)%>%
   ggplot(aes(reorder(trial_ref, grain_yield.t.ha.), grain_yield.t.ha., colour = spray_treat))+
   geom_boxplot()+
   theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.3))+
   xlab("Trial location and year")+
   ylab(bquote('Mungbean yeild  '*t~ha^-1*''))+
