...
PM_MB_dat %>%
   ...
   mutate(spray_NoSpray = factor(ifelse(fungicide_ai == "control","control","Fungicide treated"))) %>%
   mutate(control_yield = ifelse(fungicide_ai == "control",grain_yield.t.ha.,NA))%>%
   group_by(trial_ref, location, year,spray_NoSpray)%>%
   summarise(yield_c = mean(control_yield),
             yield_gain_m = mean(yield_gain, na.rm = TRUE),
             yield_gain_se = sd(yield_gain, na.rm = T)/sqrt(n()),
             .groups = "drop")%>%
   mutate(trial = paste(location,year,sep = "\n"))%>%
   mutate(yield = ifelse(spray_NoSpray == "control", yield_c, yield_gain_m))%>%
   ggplot(aes(reorder(trial_ref, yield), yield, fill = (spray_NoSpray)))+
   geom_col()+
