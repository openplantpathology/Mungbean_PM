...
Now we have the clean data let's summarise the main variables.
...
mean_control <- function(trial) {
   mean(PM_MB_dat[PM_MB_dat$trial_ref == trial &
               PM_MB_dat$fungicide_ai == "control", "grain_yield.t.ha."], na.rm = TRUE)
}

PM_MB_dat$Y_control <- sapply(PM_MB_dat$trial_ref, mean_control)

# Quantile to separate disease pressure
DP33 <- 
   PM_MB_dat %>%
   filter(fungicide_ai == "control") %>%
