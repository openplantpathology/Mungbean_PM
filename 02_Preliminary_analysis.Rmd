...
unique(PM_MB_dat[,c("location","lat","lon")])
PM_MB_dat[is.na(PM_MB_dat$lat),]

trials <- unique(PM_MB_dat, c("location","lat","lon"))
apply(trials,1, function(LL) {
   
   stat_sweep <- bomrang::sweep_for_stations(as.numeric(c(LL["lat"],LL["lon"])))
   
   # Remove banned stations
   banned_stations <- c("055286") # stations don't report rain
   stat_sweep <- stat_sweep[stat_sweep$site != banned_stations,]
   
   statID <- paste(stat_sweep[1,"name"],stat_sweep[1,"site"], sep = "_")
   
   return(statID)
   
})
bomrang::sweep_for_stations(unlist(PM_MB_dat[1,c("lat","lon")]))
...
PM_MB_dat$mid_rain <-
   apply(PM_MB_dat, 1, function(x) {
      crop_rain(
         latitude = x["lat"],
         longitude = x["lon"],
         start = ymd(x["planting_date"]) + days(20),
         end = ymd(x["planting_date"]) + days(80)
      )
   })
