...
pacman::p_load(tidyverse, lubridate, bomrang, openxlsx, devtools, reshape2, lme4)
...
Various factors have been studied in the previous experiments which may influence the following results. However unless stated the following plots and analyses were made disregarding effects from other factors not mentioned.
   ...
   4) Timing of fungicide spray/s relative to first sign of disease (_i.e_. when pathogen is present)  
...
This plot plot, however, indicated the previous plot might be skewed by high yielding trials.  
...
Volatility of the cultivars are fairly similar when trial is not considered as a factor (when we inspect plots of all treatments and only spray treatments). Green Diamond shows little volatility however this cultivar is only featured in `r length(unique(PM_MB_means[PM_MB_means$host_genotype == "Green Diamond","trial_ref"]))` trials.  
...
Lets take a closer look at Crystal with spray management treatments and without.  
```{r YieldVol_Crystal}
YV1_crystal <-
...
PM_MB_means %>%
   ...
   xlab("Row spacing (m)") +
   ylab("Grain yield - tonnes per hectare")
```
...
lm_rain1 <-
   read.csv("data/lmInSeasonRainfall_-10.09_30.64.csv",
            stringsAsFactors = FALSE)
lm_rain2 <-
   read.csv("data/lmInSeasonRainfall_-10.09_65.100.csv",
            stringsAsFactors = FALSE)
lm_rain3 <-
   read.csv("data/lmInSeasonRainfall_10.30_30.64.csv",
            stringsAsFactors = FALSE)
lm_rain4 <-
   read.csv("data/lmInSeasonRainfall_10.30_65.100.csv",
            stringsAsFactors = FALSE)
...
Can we visualise when PM is most likely to establish over the calender year?
...
season_dates[1, "Planting_day"] <-
   yday(as.Date("2012-02-28") - 7) # planted in december and emerged in February, Assuming due to lack of rain or recording error. Using 7 days prior to emergence day as planting day
...
There was some hope groups of fungicide could be combined, _i.e._ propiconazole and tebuconazole, however it seems that they have different efficacies. Let us look to see if there is anything skewing the above.
