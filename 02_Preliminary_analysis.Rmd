...
The following factors are propostulated to potentially influence mungbean grain yield and/or powdery mildew mean plot severity.
...
Now we have calculated proportional yield gains we can inpect the difference between the fungicide active ingredients
...
PM_MB_means %>%
   ...
   kable(caption = "Fungicide effect on proportion of yield gain.",
         col.names = c("Fungicide active ingredient", "Total fungicide applications","n Treatments"," 2.5% quartile","Median proportional grain yield", "Mean proportional grain yield","97.5 % quartile")) %>%
...
#### Fungicide Doses  
Hear after we will investigate only the DMI fungicides, tebuconazole and propiconazole and disregard the action of other fungicides.
...
This table shows that the majority of treatments inspected one and two spray manangement practices.  
...
To best estimate the effect of fungicide application timing we will use the day when the disease was first scouted in the field as a reference.
The first sign of disease is a useful reference as it can be assessed with little to no cost or training for growers and farm staff.  

First lets make a plot to visualise when one or more spray management regimes were made in relation to the first sign of disease regardless of fungicide active. 
...
Three spray treatments seemed to be mostly used in experiments where they were testing preventative sprays as the first fungicide application.  
...
Let's look at the volatility in yields to see if any cultivar yields better or has greater tolerance to powdery mildew. Below we plot distributions of mean trial yields for each cultivar. The following plots use data that includes all fungicide types.  
...
YV1[[3]]
```
This plot shows a large amount of yield volatility in all cultivars except green diamond. Green Diamond shows little volatility probably because this cultivar is only featured in `r length(unique(PM_MB_means[PM_MB_means$host_genotype == "Green Diamond","trial_ref"]))` trials. 
...
Volatility of the cultivars are less, albeit fairly similar, when trial is not considered as a factor (when we inspect plots of all treatments).
