...
Also the location of the trials including DMI fungicides.  
...
What is the variation in yield over all the trials in the no spray control.  

```{r ControlYield_AllTrials}
PM_MB_dat %>%
   filter(fungicide_ai == "control") %>%
   summarise(
      mean_yield = mean(grain_yield.t.ha. * 1000),
      median_yield = median(grain_yield.t.ha. * 1000),
   ...
   kable(align = 'c',
         caption = "Average yield statistics for no spray control plots")
```
...
Let's look at the yield gains in each trial compared to the no spray control, we will also calculate the proportional grain yield gains.
...
Let's show the yield gains in each trial as a table and plot
```{r YieldGain_table}
PM_MB_dat %>%
...
bw_labels[1] <- paste0(bw_labels[1], "(b)")
...
fungicide_spray_plot <-
   ...
   group_by(trial_ref, location, year) %>%
   ggplot(aes(
      reorder(trial_ref, grain_yield.t.ha.),
      grain_yield.t.ha.,
      fill = spray_treat
   )) +
   geom_boxplot() +
...
fungicide_spray_plot
#ggsave("paper/figures/Fig1_trial_yield_bw.png")
```
...
PM_MB_dat %>%
   ...
   kableExtra::add_footnote(label = "Fungicides with less than five observations were omitted from this table")
```
...
PM_MB_dat %>%
   ...
   labs(y = "Proportion of grain yield saved",
        ...
        caption = "Facets indicate number of sprays") +
...
PM_MB_dat %>%
   ...
   labs(title = "Fungicide application timing relative to First Sign of Disease",
        caption = "Facets indicate the number of applications") +
   facet_grid(cols = vars(total_fungicide)) +
   coord_flip()
```
...
PM_MB_dat %>%
   filter(year == "2016",
          host_genotype == "Jade") %>%
   mutate(Treatment = case_when(fungicide_ai == "control" ~ "control",
...
## Summary of disease incidence effect on yields
