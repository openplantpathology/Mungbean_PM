...
results_mv <- data.frame(
   ...
   eff_bar = round(sqrt(1 - (
      intercept + PMsev_mvSQ$b
   ) / intercept), 4) * 100,
   eff_CI_high = round(sqrt(
      1 - (intercept + PMsev_mvSQ$ci.ub) / intercept
   ), 4) * 100,
   eff_CI_low = round(sqrt(
      1 - (intercept + PMsev_mvSQ$ci.lb) / intercept
   ), 4) * 100
