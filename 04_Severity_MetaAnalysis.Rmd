...
data.frame(Estimates = (1-exp(PMsev_mv_log$b)),
           lowerCI = (1-exp(PMsev_mv_log$ci.lb)),
           upperCI = (1-exp(PMsev_mv_log$ci.ub)))
```
...
```{r logit_estimates}
1 - exp(PMsev_mvPer$b + PMsev_mvPer$b[1])/(1+exp(PMsev_mvPer$b + PMsev_mvPer$b[1]))
logit_estimates <- ifelse(PMsev_mvPer$b == PMsev_mvPer$b[1], PMsev_mvPer$b, PMsev_mvPer$b + PMsev_mvPer$b[1])
1 - inv_logit(logit_estimates)
...
backtransform <- function(m, input_only = FALSE, intercept) {
   if (input_only == FALSE) {
      ...
      dat <- data.frame(
         estimates = 1 - inv_logit(c(intercept, m$b[-1] + intercept)),
         ...
         CI_lb = 1 - c(inv_logit(m$ci.lb[1]), 
                       inv_logit(m$ci.lb[-1] + intercept)),
         CI_ub = 1 - c(inv_logit(m$ci.ub[1]), 
