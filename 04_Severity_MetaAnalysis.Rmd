...
PM_dat_D[PM_dat_D$yi_log == -Inf,"yi_log"] <- 0
PM_dat_D[PM_dat_D$vi_log == -Inf,"vi_log"] <- 0.001

# create column giving the respective disease severity in the control 
PM_dat_D$yi_Cp <- unlist(apply(PM_dat_D, 1, function(x1){
   
   mean(PM_dat_D[PM_dat_D$trial == x1[["trial"]] &
               PM_dat_D$spray_management == "control", "yi_percent"])
}))


# calculate response ratio
PM_dat_D$L_yi_p  <- log((PM_dat_D$yi_percent / PM_dat_D$yi_Cp) + 1)

# transform variance as per Paul et. al 2007
PM_dat_D <- 
   PM_dat_D %>%
   mutate(L_RR_v = vi *(1/replicates * (yi_percent ^ 2) + 1/replicates * (yi_Cp^2)))

#logit transformed percentages
PM_dat_D$yi_logit <- log(
   (PM_dat_D$yi_percent) / 
      (1 - (PM_dat_D$yi_percent))
   )
hist(PM_dat_D$yi_logit)
PM_dat_D$vi_logit <-
   (PM_dat_D$vi_percent) /
   (PM_dat_D$yi_percent * (1 - PM_dat_D$yi_percent)) ^ 2
   
# Correct -Inf to zeros
PM_dat_D[PM_dat_D$yi_logit == -Inf,"yi_logit"] <- 0.001
PM_dat_D[PM_dat_D$vi_logit == -Inf,"vi_logit"] <- 0.001
```
...
plot(rstandard(PMsev_mv_log)$z, ylim=c(-3,3), pch=19)
data.frame(cbind((1-exp(PMsev_mv_log$b)),
                              (1-exp(PMsev_mv_log$ci.lb)),
                              (1-exp(PMsev_mv_log$ci.ub))))
```
...
1- LaplacesDemon::invlogit(PMsev_mvPer$b + PMsev_mvPer$b[1])
mean(PM_dat_D[PM_dat_D$spray_management == "control","PM_final_severity"])
mean(PM_dat_D[PM_dat_D$spray_management == "Recommended_plus","PM_final_severity"])
```
This produces estimates which are too high and likely incorrect


```{r Metafor-Sev-analysis_logit, cache=TRUE}
#logit transformed 1- 9 values
PM_dat_D$yi_logit <- log(
   ((PM_dat_D$PM_final_severity-1)/8) / 
      (1 - ((PM_dat_D$PM_final_severity-1)/8))
   )

PM_dat_D$vi_logit <-
   (PM_dat_D$vi) /
   (((PM_dat_D$PM_final_severity-1)/8) * (1 -((PM_dat_D$PM_final_severity-1)/8))) ^ 2

# Correct -Inf to zeros
PM_dat_D[PM_dat_D$yi_logit == -Inf |
            PM_dat_D$yi_logit == Inf ,"yi_logit"] <- 0
PM_dat_D[is.na(PM_dat_D$vi_logit == Inf),"vi_logit"] <- 0

PM_dat_D[PM_dat_D$vi_logit <= 0,"vi_logit"] <- 0.001
# remove controls for response ratio meta-analysis
PMsev_mvPer <- rma.mv(
   yi = yi_logit,
   V = yi_logit,
...
1- LaplacesDemon::invlogit(PMsev_mvPer$b)
mean(PM_dat_D[PM_dat_D$spray_management == "control","PM_final_severity"])
mean(PM_dat_D[PM_dat_D$spray_management == "Recommended_plus","PM_final_severity"])
