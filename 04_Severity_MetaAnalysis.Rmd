...
intercept <- round(PMsev_mvSQ$b, 4)[1,1]
...
results_mv <- data.frame(
   ...
   N = k5,
   k = k6,
   Effect = round(sqrt(abs(PMsev_mvSQ$b)), 4),
   se = round(sqrt(abs(PMsev_mvSQ$se)), 4),
   CI_lower = round(sqrt(abs(PMsev_mvSQ$ci.lb)), 4),
   CI_upper = round(sqrt(abs(PMsev_mvSQ$ci.ub)), 4),
   z_val = round(sqrt(abs(PMsev_mvSQ$zval)), 4),
   p_val = reportP(PMsev_mvSQ$pval, AsNumeric = FALSE, P_prefix = FALSE),
   eff_bar = round(sqrt(abs(1 -(intercept + PMsev_mvSQ$b)/intercept)),4)*100,
   eff_CI_high = round(sqrt(abs(1 -(intercept + PMsev_mvSQ$ci.ub)/intercept)),4)*100,
   eff_CI_low = round(sqrt(abs(1 -(intercept + PMsev_mvSQ$ci.lb)/intercept)),4)*100
   
)

# restore negative numbers
results_mv[2:6, c(4,6:8,10:12)] <- -results_mv[2:6, c(4,6:8,10:12)]
