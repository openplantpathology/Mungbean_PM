...
# issue has something to do with website certificates not being appropriate
...
king_Tm_aver <- king_Tm_aver %>%
   mutate(
      Year = substr(king_Tm_aver$date, start = 1, stop = 4),
      Day = ymd(king_Tm_aver$date)
   )
...
war_Tm_aver <- war_Tm_aver %>%
   mutate(
      Year = substr(war_Tm_aver$date, start = 1, stop = 4),
      Day = year2day(war_Tm_aver$date)
   )
...
Herm_FS <- PM.dat %>%
   ...
   mutate(Day = year2day(First_sign))
...
Gat_Tm_aver <- Gat_Tm_aver %>%
   mutate(
      Year = substr(Gat_Tm_aver$date, start = 1, stop = 4),
      Day = year2day(Gat_Tm_aver$date)
   )
...
ggplot(subset(Gat_Tm_aver, Year == 2012),
   ...
   scale_x_continuous(
      limits = c(0, 200),
      labels = c(subset(
         Gat_Tm_aver, Year == levels(Gatton_FS$Year)
      )$date[c(1, 50, 100, 150, 200)]),
      breaks = c(1, 50, 100, 150, 200)
   ) +
   theme(
      panel.grid.major = element_line(colour = "grey80"),
