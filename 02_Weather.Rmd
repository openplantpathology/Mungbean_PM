...
season. Variation may exist in the sowing date of each trial.
...
PM.dat <-
   import_data()  # importing means yield and disease severity for each treatment within each powdery mildew trial on mungbean
source("R/year2day.r")
...
PM.dat_sum <- as.data.frame(do.call(rbind,
                                    by(PM.dat, PM.dat$trial_ref, function(x) {
                                       c(
                                          location = unique(as.character(x$location)),
                                          year = unique(as.character(x$year)),
                                          first_sign = unique(as.character(x$first_sign_disease)),
                                          day = as.numeric(unique(
                                             sapply(x$first_sign_disease, year2day)
                                          ))
                                       )
