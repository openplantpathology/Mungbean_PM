...
Below I am plotting the mean maximum temperatures over 7 days during the growing
season at each mungbean trial site.
The vertical lines indicate when powdery mildew was first observed in that
season.
...
Let's have a look at the weather for the main field experiment sites.

```{r first-sign}
# Create data subset of powdery mildew data means for the first sign of the
# disease by location
...
# Smoothing temperatures by taking a rolling 7-day average
min_T <- max_T <- vector(mode = "double", length = length(silo_king_Tmax[, 1]))

for (i in 1:length(silo_king_Tmax[, 1])) {
   max_T <- c(max_T, mean(silo_king_Tmax$max_temp[i:(i + 7)], na.rm = TRUE))
   min_T <- c(min_T, mean(silo_king_Tmax$min_temp[i:(i + 7)], na.rm = TRUE))
}
...
min_T <- max_T <- vector(mode = "double", length = length(silo_war_Tmax[, 1]))

for (i in 1:length(silo_king_Tmax[, 1])) {
   max_T <- c(max_T, mean(silo_war_Tmax$max_temp[i:(i + 7)], na.rm = TRUE))
   min_T <- c(min_T, mean(silo_war_Tmax$min_temp[i:(i + 7)], na.rm = TRUE))
}
...
# Add columns for year and day of year to data
war_Tm_aver$year <- year(war_Tm_aver$date)
war_Tm_aver$day <- yday(war_Tm_aver$date)
...
ggplot(data = subset(war_Tm_aver, war_Tm_aver$year %in% levels(Herm_FS$year)),
       aes(day)) +
   geom_line(aes(y = max_T, colour = year)) +
   geom_line(aes(y = min_T, color = year)) +
   labs(x = "Date",
        y = "Daily maximum temperature averaged over 7 days",
