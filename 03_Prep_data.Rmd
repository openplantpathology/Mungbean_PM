...
source("R/AUDPC_for_single_obs.R") # Calculate AUDPC using final severity and first sign of disease - 7 days
...
We are narrowing the meta-analysis to only include treatments that used demethylation inhibitors, tebuconazole and propiconazole. 
...
First lets get a list of trials that reported LSD.  
```{r all_LSD_trials}
Trials <- PM_MB_means %>%
...
Two or more sprays where the first spray was either early, recommended timing, or late.
...
slimmer_PM_dat <-
   slim_PM_dat[slim_PM_dat$spray_management != "Late" &
...
The linear model confirms a significant positive relationship between grain yield and AUDPC. 
