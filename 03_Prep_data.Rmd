...
for (i in Trials$trial_ref) {
   ...
   V_yield <-
      (PM_MB_means[PM_MB_means$trial_ref == i,]$replicates[1] * (LSD /
...
This might be due to the unpredictable nature of mungbeans, however going forward we will use mean squares which defines between trial variation and is a the more generic error term.
...
Using the model without dose produces a lower AIC, however we should accept the data with dose as there is a significant difference between the models and we should accept the more complicated model `m2`. 
We will try and evaluate dose in the meta-analysis, however for now we will remove the dose factor to reduce the possibility of overfitting the model.
...
We may want to consider row spacing in the trials, However one of the locations has NA as the row spacing. 
For now let's assign the most used row spacing of 0.33 m.
