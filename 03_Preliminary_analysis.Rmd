...
# Rewrite all variations of sulphur treatments as just sulphur
PM_MB_means$Fungicide[PM_MB_means$Fungicide == "Sulphur"| PM_MB_means$Fungicide == "Sulphur (SC)" | PM_MB_means$Fungicide == "Sulphur (Ultra SC)" | PM_MB_means$Fungicide == "Sulphur (WP)"] <- "Sulphur"
```
...
PM_MB_means %>%
   filter(Fungicide == c("200 g/l  Tebuconazole + 120 g/l  Azoxystrobin" , "Amistar","Tebeconazole","propiconazole"))%>%
...
```
There was some hope groups of fungicide could be combined, ie Propiconazole and Tebeconazole, however it seems that they have different efficacies. Let us look to see if there is anything skewing the above.
...
PM_MB_means %>%
   ...
   filter(Fungicide != "Pyrazophos" & Fungicide != "control" & Fungicide != "Benomyl" & Fungicide != "Acibenzolar-S-Methyl" & Fungicide != "200g/L azoxystrobin + 80 g/L cyproconazole + adj" ) %>%
ggplot(aes(y = prop_YG, x = Fungicide))+
   facet_grid(rows = vars(total_Fungicide))+
   geom_boxplot(aes(fill = Fungicide, alpha = 0.5))+
   geom_hline(aes(yintercept = 0), size =0.5)+
   #scale_fill_discrete(labels = c("15-7 days before","same day","one day after","8-9 days after","13-19 days after"))+
   labs(y = "Proportion of grain yeild saved",
   title = "Grain yield proportion saved\ngrouped by Fungicide type")+
   theme_usq()+
   theme(legend.position="none")+
   coord_cartesian()+
   scale_x_discrete(limits = c("200 g/l  Tebuconazole + 120 g/l  Azoxystrobin","propiconazole", "Tebeconazole", "200g/L azoxystrobin + 80 g/L cyproconazole", "Sulphur", "Carbendazim", "Pyraclostrobin","Amistar"))+
...
PM_MB_means$Fungicide <- factor(PM_MB_means$Fungicide, levels = c("control","propiconazole","Tebeconazole",
                                                        "Amistar",
                                                        "200 g/l  Tebuconazole + 120 g/l  Azoxystrobin", "Carbendazim",
                                                        "Benomyl",  
                                                         "Pyrazophos","Acibenzolar-S-Methyl",
                                                        "Sulphur", "Sulphur (SC)","Sulphur (Ultra SC)","Sulphur (WP)"
                                                        ))

PM_MB_lmer.1 <- lmer(prop_YG ~ Fungicide * as.factor(total_Fungicide) + row_spacing + (1|Trial_ref), 
     data = subset(PM_MB_means, total_Fungicide == 1 | total_Fungicide == 2))
...
Tebe_trials <- unique(PM_MB_means[PM_MB_means$Fungicide == "Tebeconazole",]$Trial_ref)
Propi_trials <- unique(PM_MB_means[PM_MB_means$Fungicide == "propiconazole",]$Trial_ref)
Amistar_trials <- unique(PM_MB_means[PM_MB_means$Fungicide == "Amistar",]$Trial_ref)

intersect(
   intersect(Tebe_trials,Propi_trials)
   ,Amistar_trials)
...
#   filter(total_Fungicide ==1) %>%
   mutate(days_from_1st = as.numeric(as.Date(as.character(Fungicide_application_1), format = "%d/%m/%Y") - 
...
for(i in 1:dim(PM_MB_means1)[1]){
   dF1 <- as.numeric(as.Date(PM_MB_means1$Fungicide_application_1, format = "%d/%m/%Y")[i] -
                        as.Date(PM_MB_means1$first_sign_disease, format = "%d/%m/%Y")[i])
   dF2 <- as.numeric(as.Date(PM_MB_means1$Fungicide_application_2, format = "%d/%m/%Y")[i] -
                        as.Date(PM_MB_means1$first_sign_disease, format = "%d/%m/%Y")[i])
   dF3 <- as.numeric(as.Date(PM_MB_means1$Fungicide_application_3, format = "%d/%m/%Y")[i] -
                        as.Date(PM_MB_means1$first_sign_disease, format = "%d/%m/%Y")[i])
   if(is.na(dF1)){
...
PM_MB_means1$spray_diff <- vec1 # gives the closest fungicide application date to the first sign of disease
