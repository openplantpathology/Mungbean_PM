...
PM_MB_dat %>%
   ...
   ylab("Fungicide application timing relative to First Sign of Disease") +
   facet_grid(cols = vars(total_fungicide)) +
   coord_flip()
```
...
Green Diamond shows little volatility probably because this cultivar was only used in `r length(unique(PM_MB_dat[PM_MB_dat$host_genotype == "Green Diamond","trial_ref"]))` trial. 
...
SeasonPlot <- season_dates %>%
   ...
   geom_point(aes(x = trial_ref, y = Disease_day), size =2.5) +
   coord_flip() +
...
In most years, regardless of the age of the crop, powdery mildew establishes in March, which is to be expected.
The disease is driven by weather conditions and the plant is vulnerable at any growth stage.
...
The trials `r unique(PM_MB_dat[is.na(PM_MB_dat$AUDPC_m),"trial_ref"]) ` don't have an estimate of AUDPC and need to be filled in. 
...
write.csv(PM_MB_dat,
          "cache/1911_PM_MB_dat&Ygains.csv", row.names = FALSE)
```
...
plot(grain_yield.t.ha. ~ AUDPC_m, data = PM_MB_dat)
abline(lm(grain_yield.t.ha. ~ AUDPC_m, data = PM_MB_dat))
...
```{r AUDPClm}
summary(lm(grain_yield.t.ha. ~ AUDPC_m, data = PM_MB_dat))
```

This is not what we would expect and the low R squared value of `r round(summary(lm(grain_yield.t.ha. ~ AUDPC_m, data = PM_MB_dat))$adj.r.squared,3)` perhaps there are additional factors influencing this relationship.
...
for (i in sm2lg$trial_ref) {
   ...
   plot(
      ...
      ylim = c(0, max(PM_MB_dat$grain_yield.t.ha., na.rm = TRUE)),
      xlim = c(0, max(PM_MB_dat$AUDPC_m, na.rm = TRUE))
   )
...
dat3 <- PM_MB_dat %>%
   filter(year == "2016",
          ...
          fungicide_ai == "control")%>%
   group_by(location)%>%
   summarise(C_Yield = mean(grain_yield.t.ha., na.rm = TRUE))
   
dat3$F_Yield <- PM_MB_dat %>%
   filter(year == "2016",
          host_genotype == "Jade",
          fungicide_ai != "control")%>%
   group_by(location)%>%
...
dat3 %>% 
   mutate(diff = F_Yield - C_Yield) %>%
   mutate(percent = (diff/F_Yield)*100)
...
```{r Yield_vs_AUDPS}
plot(grain_yield.t.ha. ~ AUDPS_m, data = PM_MB_dat)
abline(lm(grain_yield.t.ha. ~ AUDPS_m, data = PM_MB_dat))

summary(lm(grain_yield.t.ha. ~ AUDPS_m, data = PM_MB_dat))
...
for (i in sm2lg$trial_ref) {
   ...
   plot(
      ...
      main = paste(dat1$location[1], (dat1$year[1])),
      ylim = c(0, max(
         PM_MB_dat$grain_yield.t.ha., na.rm = TRUE
      )),
      xlim = c(0, max(PM_MB_dat$AUDPS_m, na.rm = TRUE))
   )
...
```{r}
save(SeasonPlot, file = here("cache/PreliminaryData.Rdata"))
```
