...
```{r}
mpg %>%
   group_by(manufacturer,drv,cyl)%>%
   filter(n() >= 2)%>%
   summarise(mean_Sp = mean(cty),
             stdev_h = sd(cty),
             n_group = n())
...
Mean powdery mildew severity was recorded at the end of each growing season.
Some experiments recorded disease severity throughout the season and these were summarised with AUDPC.
AUDPC for trials which only provided final powdery mildew severity were calculated using the record of 'first sign of disease' and final assessment.
...
First let's check if we have all the AUDPC observations by searching for trials with mission AUDPC.
...
```{r AUDPCxTrial, fig.height= 10, fig.width=10}
mTrialYield <- PM_MB_dat %>%
   group_by(trial_ref) %>%
   mutate(TY = mean(grain_yield.t.ha., na.rm = TRUE)) %>%
   pull(TY)

PM_MB_dat$trial_ref <- reorder(PM_MB_dat$trial_ref, mTrialYield)
...
PM_MB_dat %>%
   ...
   ggplot(aes(y = grain_yield.t.ha., x = AUDPC_m))+
   geom_point(aes(color = Treatment))+
   facet_wrap(vars(trial_ref))+
   scale_y_continuous(limits = c(0,3))+
   geom_smooth(method = "lm", se = FALSE, linetype = 1, colour = "grey", size =1)+
   scale_colour_usq()
