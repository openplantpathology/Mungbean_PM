...
PM_dat_Y <- PM_dat_Y %>%
   mutate(first_sign_days = lubridate::ymd(first_sign_disease) -
             lubridate::ymd(planting_date)) %>%
   mutate(onset = case_when(first_sign_days <= 42 ~ "early_onset",
...
Also the chi-squared test uncertain that the `onset` model is significantly better as indicated by the `pval` `r round(anova(PM_mv, PM_mv_onset)$pval,3)`.
