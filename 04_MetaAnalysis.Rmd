...
Next we will test whether including `spray_management` as a significantly accounts for enough variation as a fixed effect variable to be included in the model.  
Finally we will test whether including `spray_management` as a significantly accounts for enough variation as a random slope to the trial variable.  
...
The best model from the four above is `m11` with the lower AIC of -`r min(anova(m9, m11)$AIC)` 
...
```{r imputing_variances, eval=FALSE, include=FALSE}
...
The use of these imputed MSE was giving a varied outcome each time it was run. We the next code chunk removes the studies without variation to observe what the meta-analysis outcome would be.
...
According to the results of the omnibus test ($Q_M = 10.65, df = 4, p = 0.0308$) we can reject the null hypothesis $H_0 : \beta_1 = \beta_2 = \beta_3 =\beta_4 = 0$ that the tested spray management schedules have no benefit to protecting mungbean yields[@Viechtbauer2010]. 
However Late plus ($\beta_4$) was the only moderator showing a significant effect on yields ($z = 3.2276; p = 0.0012$). 
The analysis shows there is still a considerable amount of residual heterogeneity ($Q_E = 5096.04, df = 125, p < 0.0001$) not captured by the spray management moderator indicating other possible moderators which might influence grain yield.  
