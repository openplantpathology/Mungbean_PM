...
```{r forestP, fig.height= 40, fig.width=10}
# If anyone knows how to fix the margins here it would be helpful!!!!!!!!!!!!!!!!
metafor::forest(PM_mv, cex =0.75 , width = 5,
                ilab = cbind(dat1$trial_ref, dat1$location, dat1$year, dat1$row_spacing, dat1$host_genotype, dat1$dose,
                             as.character(dat1$spray_management)),
                ilab.xpos = c(-6.5, -5.5,-4.75,-4.15,-3.4,-2.5,-1),
                slab = NA, 
                xlim = c(-7, 3.5),
                ylim = c(4,119),
...
text(c(-6.5, -5.5,-4.75,-4.15,-3.4,-2.5,-1), 117.9,cex = 0.85, 
     c("Trial\nReference", "Location", "Year", "Row\nSpacing","Host\nCultivar","DMI dose","SM"),
     font = 2)
