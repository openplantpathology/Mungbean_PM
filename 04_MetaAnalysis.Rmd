...
Also the chi-squared test uncertain that the `d_pressure` model is significantly better as indicated by the `pval` 0.1336.
The higher log likelihood on the `d_pressure` model does intricate there is some merit to this model and that perhaps with more trials to increase statistical power might provide a useful model.  
...
summary(glht(PM_mv_, linfct = cbind(contrMat(rep(
   1, 6
), type = "Tukey"))), test = adjusted("none"))
contrast_Ssum <-
   simple_summary(summary(glht(PM_mv_, linfct = cbind(
