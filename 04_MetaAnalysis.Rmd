...
A secondary question to this aim is: given the time at which the first spray occurred is a second spray provide worthwhile yield protection.
...
Lets try the model with an implied simpler variance-covariance matrix "compound symetry" ("CS") to examine if unstructured matrix is suitable.
```{r}
PM_mv_cs <- rma.mv(
   yi = grain_yield.t.ha.,
   vi,
...
k5 <- as.data.frame(table(PM_MB_dat$trial, PM_MB_dat$spray_management)) %>%
   filter(Freq !=0)%>%
   group_by(Var2)%>%
   summarise(n())%>%
   pull()

results_mv <- data.frame(N = PM_mv$g.levels.k,
                         k = k5,
...
colnames(results_mv)[c(1:4,6:9)] <- c("Treatment","$N$", "$k$","$\\mu$","$CI_{L}$","$CI_{U}$", "$Z$","$P$")

kable(results_mv[,2:9],
      align = "lcccccccc",
      booktabs = TRUE,
      caption = "Table ##: Mean effect size of five spray schedule treatments on mitigating yield losses to powdery mildew in mungbean crops grown in eastern Australia. Effect sizes were estimated from the network meta-analysis of data obtained from grey literature reports. Intercept indicates the mean of the no spray control and the treatments show the mean difference to the intercept")
...
results_mv %>%
   ...
   ylab(expression(paste("Mean yield difference to control (t ha" ^ -1,")",sep = ""))) +
   scale_x_discrete(
      "Moderator variable",
      labels = c(
...
Lets investigate the treatment contrasts
...
forest1  <- metafor::forest(PM_mv, cex = 1.15 , width = 5,
                ...
                ilab = cbind(PM_MB_dat$year, 
                             PM_MB_dat$row_spacing, PM_MB_dat$host_genotype, PM_MB_dat$dose),
...
save(efficacy, PM_MB_dat, PM_mv, contrast_Ssum, 
