...
```{r forestP, fig.height= 25, fig.width=20}
...
polys <- data.frame(ref = change_labels(names(coef(PM_mv))),
                    ...
                    ci.lb = PM_mv$ci.lb,
                    ci.ub = PM_mv$ci.ub,
                    rows = c(120.60, 68.6, 24.6,99.6, 1.6))

metafor::forest(PM_mv, cex = 1.15 , width = 5,
                order = "fit",
                rows = c(134:122, 117:101, 96:70, 65:26, 21:3),
                ilab = cbind(dat1$trial_ref, dat1$location, dat1$year, 
                             dat1$row_spacing, dat1$host_genotype, dat1$dose),
                ilab.xpos = c(-3.3, -2.85,-2.55,-2.35,-2.15,-1.85),
                slab = NA, 
                xlim = c(-3.5, 2.5),
                ylim = c(4,137),
...
text(c(-3.3, -2.85,-2.55,-2.35,-2.15,-1.85, -1.5), 137.9,cex = 1, 
     c("Trial\nReference", "Location", "Year", "Row\nSpacing","Host\nCultivar","DMI dose"),
     font = 2)

text(-2.35, polys$rows ,cex = 1.3, 
