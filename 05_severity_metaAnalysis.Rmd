...
```{r noDiseaseError}
unique(PM_dat_D[is.na(PM_dat_D$d_vi),
                      c("trial_ref", "location", "year")])
```

Eliminate trials with out disease severity variance.
```{r rm_SevDiseaseError}
PM_sev <- 
   PM_dat_D %>%
   filter(is.na(d_vi) == FALSE)
```
Are all the reported disease variance the same type?
```{r sev_var_type}
unique(PM_sev[,"D_error_type"])
```

Now we have eliminated 2 trials do all spray management categories have sufficient replications
```{r treatment_n}
...
Convert variance to the same measure
```{r calc_sampleVar}
PM_sev <- 
   PM_sev %>%
   mutate(
